  ...
  <div class="px-4 py-6 max-w-4xl mx-auto w-full pb-16">
    ...
    <div class="flex items-center justify-between mb-5">
      ...
      <button
        ...
        class="w-9 h-9 flex items-center justify-center rounded-lg text-vault-muted hover:text-vault-accent hover:bg-vault-card transition-colors disabled:opacity-30 disabled:cursor-default"
      >
        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    <template v-else>
      ...
      <div class="bg-vault-card border border-vault-border rounded-2xl p-5 mb-5">
        <div class="grid grid-cols-2 gap-4 mb-4">
          <div>
            ...
            <p class="font-serif text-xl text-green-400">{{ formatIDR(totalIncome) }}</p>
          </div>
          <div>
            <p class="text-[10px] text-vault-muted uppercase tracking-wider mb-1">Pengeluaran</p>
            <p class="font-serif text-xl text-red-400">{{ formatIDR(totalExpense) }}</p>
          </div>
        </div>
        ...
        <div class="border-t border-vault-border pt-4">
          ...
          <div v-if="totalIncome > 0" class="space-y-1">
            <div class="h-2 bg-vault-border rounded-full overflow-hidden">
              <div
                ...
                :class="expenseRatio > 90 ? 'bg-red-400' : expenseRatio > 70 ? 'bg-amber-400' : 'bg-green-400'"
          ...
          <div v-else-if="totalExpense > 0" class="mt-1">
            <p class="text-[10px] text-amber-400">Belum ada pemasukan dicatat bulan ini</p>
          </div>
        </div>
      </div>
      ...
      <template v-else>
        ...
        <div v-if="categoryBreakdown.length > 0" class="mb-5">
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="h-1 bg-vault-border rounded-full overflow-hidden">
                  <div
                    ...
                    :style="{ width: (item.amount / categoryBreakdown[0].amount * 100) + '%', backgroundColor: item.color }"
                  />
                </div>
              </div>
            </div>
          ...
          <div v-for="group in groupedByDate" :key="group.date">
            ...
            <div class="flex items-center justify-between mb-2">
              ...
              <span
                ...
                :class="group.dayNet >= 0 ? 'text-green-400' : 'text-red-400'"
              >
            ...
            <div class="bg-vault-card border border-vault-border rounded-xl overflow-hidden">
              ...
              >
                <div
                  ...
                  :style="{ backgroundColor: getCatColor(trx.category, trx.type) + '22' }"
                >
                ...
                <div class="flex items-center gap-2 shrink-0">
                  <span
                    class="text-sm font-semibold"
                    :class="trx.type === 'income' ? 'text-green-400' : 'text-red-400'"
                  >
                    {{ trx.type === 'income' ? '+' : '-' }}{{ formatIDR(trx.amount) }}
                  </span>
                  <button
                    @click.stop="removeTransaction(trx.id)"
                    class="opacity-0 group-hover:opacity-100 text-vault-muted hover:text-red-400 transition-all w-7 h-7 flex items-center justify-center"
                  >
        ...
        <div class="relative bg-vault-card border border-vault-border rounded-t-2xl sm:rounded-2xl w-full sm:max-w-md flex flex-col max-h-[92vh]">
          ...
          <div class="flex-1 overflow-y-auto px-5 pb-5 space-y-5">
            ...
            <div class="grid grid-cols-2 gap-2 bg-vault-bg rounded-xl p-1">
              <button
                ...
                :class="formType === 'expense' ? 'bg-red-500/15 text-red-400 shadow-sm' : 'text-vault-muted hover:text-vault-text'"
              >
                Keluar
              </button>
              <button
                ...
                class="py-2.5 rounded-lg text-sm font-semibold transition-all"
                :class="formType === 'income' ? 'bg-green-500/15 text-green-400 shadow-sm' : 'text-vault-muted hover:text-vault-text'"
              >
                Masuk
              </button>
            </div>
            ...
            <div class="bg-vault-bg border border-vault-border rounded-xl px-5 py-4 text-center">
              ...
              <input
                ...
                min="0"
                placeholder="0"
                class="w-full bg-transparent text-3xl font-serif text-center text-vault-text focus:outline-none placeholder:text-vault-muted/30"
                inputmode="numeric"
              />
              ...
              <div class="grid grid-cols-4 gap-2">
                <button
                  ...
                  :class="formCategory === cat.key
                    ...
                    : 'border-vault-border bg-vault-bg hover:border-vault-accent/30'"
                  :style="formCategory === cat.key ? { backgroundColor: cat.color + '22', borderColor: cat.color + '66' } : {}"
                >
            ...
            <div>
              ...
              <input
                v-model="formNote"
                placeholder="Contoh: Makan siang sama temen..."
            ...
            <div>
              ...
              <input
                v-model="formDate"
                type="date"
                class="bg-vault-bg border border-vault-border rounded-xl px-4 py-2.5 text-sm text-vault-text focus:outline-none focus:border-vault-accent/30 transition-colors"
              />
            </div>
          ...
          <div class="px-5 pb-6 pt-3 border-t border-vault-border shrink-0">
            <button
              ...
              :disabled="!formAmount || Number(formAmount) <= 0 || !formCategory || saving"
              class="w-full py-3 rounded-xl text-sm font-semibold transition-colors disabled:opacity-30"
              :class="formType === 'expense' ? 'bg-red-500/80 hover:bg-red-500/70 text-white' : 'bg-green-500/80 hover:bg-green-500/70 text-white'"
            >
...
// ── Category breakdown (top 4 expenses) ───────────────────────────────────
const categoryBreakdown = computed(() => {
  ...
  return Object.entries(map)
    .map(([key, amount]) => ({ key, amount, ...getCatMeta(key, 'expense') }))
...
const activeCategories = computed(() =>
  formType.value === 'expense' ? expenseCategories : incomeCategories
)

// Reset category when switching type if incompatible
watch(formType, (newType) => {
  const cats = newType === 'expense' ? expenseCategories : incomeCategories
  if (!cats.find(c => c.key === formCategory.value)) {
    formCategory.value = cats[0].key
  }
})
...
const saveTransaction = async () => {
  ...
  // Check if the transaction's date falls in the current viewed month
