...
const handleSave = async (data: { raw: string; tag: string; reminderAt?: string | null; projectId?: string | null; title?: string | null }) => {
  saving.value = true
  ...
  try {
    ...
    if (editingNote.value) {
      ...
      const note = await createNote({ raw: data.raw, tag: data.tag || '', project_id: data.projectId, title: data.title || undefined })
